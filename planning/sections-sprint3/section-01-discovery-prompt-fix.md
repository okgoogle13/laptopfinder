...
1. `make test` — green (one new assertion in test_prompt_markers.py)
2. `make inject-config` — `comet_discovery_agent.txt` UMA line shows `32`
3. Grep confirms no raw `64GB+` remains in the UMA description line
