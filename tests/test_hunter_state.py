import json
from laptopfinder.runners.hunter.state import load_seen, save_seen, write_jsonl

def test_load_seen_empty(tmp_path, monkeypatch):
    monkeypatch.setattr("laptopfinder.runners.hunter.state.SEEN_PATH", tmp_path / "seen_items.json")
    assert load_seen() == set()

def test_save_and_load_seen(tmp_path, monkeypatch):
    p = tmp_path / "seen_items.json"
    monkeypatch.setattr("laptopfinder.runners.hunter.state.SEEN_PATH", p)
