...
from laptopfinder.runners.hunter.llm import triage_corpus, recover_vram_from_images

@patch("laptopfinder.runners.hunter.llm.gemini_client")
def test_triage_corpus_empty(mock_client):
    res = triage_corpus(mock_client, "model", [])
    assert res == {}

@patch("laptopfinder.runners.hunter.llm._gemini_json")
...
def test_recover_vram_from_images(mock_client):
    ...
    with patch("laptopfinder.runners.hunter.llm._gemini_json", return_value={"vram_gb": 16, "verbatim_quote": "16GB GDDR6", "confidence": 1.0}):
        with patch("laptopfinder.runners.hunter.llm._fetch_image_bytes", return_value=(b"A"*20000, "image/jpeg")):
